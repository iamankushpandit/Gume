...
void DiceGame::renderStatic(AppContext& host) {
...
void DiceGame::renderDynamic(AppContext& host) {
    Ui::Renderer& tft = host.display();

    /* The band is wiped because the dice have to be: a three-dice layout leaves
     * stale pips behind when you drop to one.
     *
     * But while the dice are tumbling the line underneath them says
     * "Rolling..." and keeps saying it, so clearing the whole band every frame
     * of the animation repaints that text for nothing. A tumbling frame that
     * follows another tumbling frame clears only the dice. Everything else --
     * the first frame of a roll, the last one, a change of dice count -- takes
     * the full band, which is what puts "Total N" over "Rolling...". */
    const bool tumbling = rolling_ && drawnRolling_;
    const Rect band = activeBand();
    if (tumbling) {
        tft.fillRect(band.x, band.y, band.w, 76, Ui::bg());
    } else {
