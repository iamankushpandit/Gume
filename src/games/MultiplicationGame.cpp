...
Rect MultiplicationGame::answerRect(uint8_t index, int16_t screenW) const {
...
void MultiplicationGame::render(AppContext& host) {
    ...
    tft.drawString(bestStreak_ > 0 ? String("Best ") + bestStreak_ : "Best --", W - 10, 52, 1);
    ...
    if (answered_) {
        ...
        tft.setTextDatum(MC_DATUM);
        tft.drawString("Tap the product", W / 2, 136, 2);
    }
    tft.setTextDatum(TL_DATUM);
}
