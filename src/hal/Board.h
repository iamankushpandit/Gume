...
public:
    ...
    /* THIS DEVICE'S OWN NAME FOR ITSELF -- eight hex characters, generated by
     * the firmware on first boot and kept in NVS.
     *
     * It exists so that identifying a board never requires its MAC. A MAC is
     * burned into eFuse: it cannot be changed, so publishing one publishes a
     * permanent identifier for a specific piece of hardware in somebody's
     ...
     * NOT survive a merged-image install (which paves the NVS region) and it
     * does not exist at all on a board with no firmware. A MAC answers in both
     * of those cases, which is why esptool still reads one when a board cannot
     * introduce itself -- locally, on the machine holding the hardware, and
     * never into a file this repository tracks. */
    const char* deviceId();
...
private:
    ...
    /* Tag, hyphen, eight hex characters, terminator. Mirrored because the boot
     * banner, System Info and any tool asking "which board is this?" all read
     * it, and it never changes while the device is up. */
    char cachedDeviceId_[20] = {0};
