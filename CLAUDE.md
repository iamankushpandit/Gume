...
Flash is global and nearly the binding constraint (2,539,625 / 3,145,728 bytes,
**80.7%**; NimBLE plus the BT controller account for ~192 KB of that). RAM sits
...
- **`APP_REGISTRY` holds the 38 playable games plus 7 launchable system apps.** The launcher itself is not a tile in that table; it is `LauncherGame`, activated by `goHome()`.
...
3. The build figures in `README.md` and `CLAUDE.md` are from a build of the
   ...
   **`BRAINO_VERSION` is in the image too, so measure after the version bump,
   ...
   2,376,053 on `5.6.0-SNAPSHOT` against 2,371,981 on `5.5.1` -- and that is
