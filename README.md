...
The ST7789 / CYD2USB build is offered for the later two-USB version of the same
2.8-inch CYD board, but it has not been run on hardware here. Other ESP32 boards
may accept the binary, but their display and touch pins will not match this
firmware.

> **Use it at your own risk.** This is an open-source hobby project, not a
> product. It comes with **no warranty and no guarantee of functionality**, and
> the authors accept **no liability** for damage, data loss or an unusable
> board — see [LICENSE](LICENSE) and [Supported boards](#supported-boards).
> Flashing overwrites whatever is on the device.

That page is generated by `tools/gen_site.py` and published by
`.github/workflows/pages.yml`, which builds the browser-offered firmware
environments on every push to `main` and puts the real binaries behind the
button. The page
...
Only the 2.8-inch CYD is supported. The two panel revisions of that board are
electrically identical — same ESP32, same GPIOs, same XPT2046 resistive touch
— and differ only in the display controller, which is a compile flag:

| Board | Panel | Firmware | Status |
|---|---|---|---|
| ESP32-2432S028R, single micro-USB (sold as **E32R28T-1** / ESP32-32E) | ILI9341 | `app`, `bringup` | **Tested.** The board this was written on |
| ESP32-2432S028R, micro-USB + USB-C (**CYD2USB**, v2/v3) | ST7789, BGR order, inversion off | `app_st7789`, `bringup_st7789` | **Untested.** Compiles, never run on hardware |

Wrong panel flags do not stop it booting — they give inverted or blue-tinted
colours, or a blank screen. That is what the bring-up build is for.

**"Untested" is literal.** Only the ILI9341 board has ever been powered on
here. The ST7789 build is compiled against the published pin map for that
revision and nothing more; it may show wrong colours, misplaced touch, or fail
to start. If you run it, please open an issue saying whether it worked — that
is the only way it stops being untested.

### Boards that are *not* supported, and why

- **ESP32-2432S024R (2.4-inch).** Looks like a close relative, is not one: its
  XPT2046 shares the display's SPI bus, the backlight moves to GPIO 27, and
  GPIO 34 is a light sensor rather than a battery divider. `Board::pollTouch()`
  bit-bangs dedicated touch pins, so this needs driver work rather than a flag,
  and a build for it would read the LDR as a battery.
- **3.5-inch boards (ESP32-3248S035R/C).** 480×320 panels. Every game is
  authored against a fixed 320×240 canvas, so these need the canvas
  letterboxed in `Board`/`Ui` — and the `-C` variants need a GT911 or CST820
  capacitive driver as well.
- **ESP32-S3 boards (8048S043 and friends).** Different chip family, RGB
  parallel panel, PSRAM. Effectively a second port of the HAL.
...
```bash
pio run -e app -t upload            # ILI9341 board (the default env)
pio run -e app_st7789 -t upload     # ST7789 / two-USB board
```

Requires **PlatformIO**. The panel flags live in the `[panel_*]` sections of
`platformio.ini`; everything else about the two boards is identical, which is
why they share `[common]` and `[env_base]`. The `huge_app.csv` partition (3 MB) is already set in
...
The app builds above are also on the [web installer](#install-it-without-a-toolchain)
— useful when the board is not on the machine that has PlatformIO. The
diagnostic builds below are source-toolchain builds only, because they replace
the console with serial test firmware.
...
A display and touch check, per panel, that replaces the app entirely:

```bash
pio run -e bringup -t upload          # ILI9341
pio run -e bringup_st7789 -t upload   # ST7789
```

An isolated Wi-Fi radio test, built with **no** display, touch or game code —
one build serves every panel:
...
That licence is also the warranty position: the software is provided **as is,
without warranty of any kind**, and the authors are **not liable** for any
claim or damages arising from it. Publishing the source makes the firmware
checkable; it does not make it guaranteed.
