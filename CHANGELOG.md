...
sources -- LCDWIKI, Freenove, Random Nerd Tutorials -- each with the date
checked and a Wayback Machine copy where one exists, because vendor pages
move and vanish. The facts table, the GPIO table and a pin diagram on each
page are generated by `tools/gen_board_docs.py` from the board's profile and
its `platformio.ini` section, the same two files the firmware is built
from; `--check` runs in CI. The pictures are our own: the vendors state no
licence for theirs, so they are linked rather than copied. Photos of the
boards can be added to `docs/boards/img/`; the tool strips their metadata --
EXIF can hold a GPS position or a name -- and CI refuses one that still
carries any.

**Still outstanding, carried over from 5.9.0 and 5.10.0:** a two-console
regression check of nearby play -- which is why About now calls two-console
play experimental rather than letting the owner find out. Two defects in that path were fixed late in
...
**Also outstanding: the boot banner should carry the device's stored STATE,
not just its identity.** Diagnosing the 5.9.1 panel bug took far longer than it should
